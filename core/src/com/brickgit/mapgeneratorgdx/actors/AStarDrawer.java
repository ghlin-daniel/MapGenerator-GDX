...
import com.brickgit.mapgeneratorgdx.algorithms.AStarAlgorithm;
...
public class AStarDrawer extends BaseDrawer {
    ...
    public void drawStepByStep(Batch batch) {
        ...
        if (!asa.isFinished()) {
            handlingPosition = asa.searchStepByStep();
        } else if (path == null) {
            path = asa.getPath();
            ...
            for (int x = 0; x < map.getSize(); x++) {
                ...
                switch (map.getMapRole(position)) {
                    ...
                    case SPACE:
                        ...
                        double cost = asa.getCost(position);
    ...
    private void drawCost(Batch batch, double cost, int x, int y, boolean isOnPath) {
        bitmapFont.getData().setScale(0.5f);
        bitmapFont.setColor(isOnPath ? Color.RED : Color.WHITE);
        GlyphLayout glyphLayout = new GlyphLayout();
        glyphLayout.setText(bitmapFont, String.valueOf((int) cost));
        bitmapFont.draw(
                batch,
                glyphLayout,
